...
public class SubReportBuilder {
    ...
    public SubReportBuilder addParameterFieldType(String propertyName, String paramName) {
        SubreportParameter sp = new SubreportParameter(paramName, propertyName, null,
     ...
     *
     * @param origin
     *            Must be one of these constans located in DJConstants inteface<br>
     *            - SUBREPORT_DATAS_OURCE_ORIGIN_PARAMETER<br>
     *            - SUBREPORT_DATAS_OURCE_ORIGIN_FIELD<br>
     *            - SUBREPORT_DATAS_OURCE_ORIGIN_INTERNAL<br>
     * @param type
     *            tell if the datasource is a Collection, an Array, a ResultSet or
     *            whatever.<br>
     *            Its value must be a constant from {@link DJConstants} of the like
     *            DATA_SOURCE_TYPE_...
     * @param expression
     *            is -depending on the origin- te path to the datasource<br>
     *            ie: if origin is SUBREPORT_DATAS_OURCE_ORIGIN_PARAMETER, then
     *            expression can be "subreport_datasource".<br>
     *            You must in the parameters map an object using
     *            "subreport_datasource" as the key.<br>
     *            The object must be an instance of {@link JRDataSource} or any of
     *            the following<br>
     *            Collection, Array, ResultSet, or any of the data source types
     *            provided by Jasper Reports
     * @return
     */
    ...
    /**
     * like addDataSource(int origin, int type, String expression) but the type will
     * be of the {@link JRDataSource}
     * 
     * @param origin
     * @param expression
     * @return
    ...
    /**
     * like addDataSource(int origin, String expression) but the origin will be from
     * a Parameter
     * 
     * @param expression
     * @return
    ...
    public SubReportBuilder setDataSource(String expression) {
        return setDataSource(DJConstants.DATA_SOURCE_ORIGIN_PARAMETER, DJConstants.DATA_SOURCE_TYPE_JRDATASOURCE,
                expression);
    }
    ...
    /**
     * When true, and id the subreport is dynamic, it's page size and margins will
     * match with the parent report
     * 
     * @param fitparent
     * @return
     */
    ...
    /**
     * defines the KEY in the parent report parameters map where to get the
     * subreport parameters map.
     * 
     * @param path
     *            where to get the parameter map for the subrerpot.
     * @return
     */
     ...
     *            where to get the parameter map for the subrerpot
     * @param origin
     *            where the paramters map comes from:
     *            DJConstants.SUBREPORT_PARAMETER_MAP_ORIGIN_PARAMETER or
     *            DJConstants.SUBREPORT_PARAMETER_MAP_ORIGIN_FIELD
     * @return
     */
    ...
    /**
     * If false, and the report doesn't fit in the space given until end of page, it
     * will be pushed to the next page. default is TRUE
     * 
     * @param splitAllowed
     * @return
     */
