...
public class CrosstabBuilder {
    ...
    /**
     * Add a column to the crosstab. In a double entry "X\Y" like table table,
     * columns are "Y" (rows are X) The first column added will be the inner most
     * one.
     * 
     * @param col
     * @return
    ...
    public CrosstabBuilder addColumn(String title, String property, String className, boolean showTotal) {
        DJCrosstabColumn col = new CrosstabColumnBuilder().setProperty(property, className).setShowTotals(showTotal)
    ...
    public CrosstabBuilder addColumn(String title, String property, String className, boolean showTotal,
            String totalLegend) {
        DJCrosstabColumn col = new CrosstabColumnBuilder().setProperty(property, className).setShowTotals(showTotal)
                .setTotalLegend(totalLegend).setTitle(title).build();
        addColumn(col);
        return this;
    }
    ...
    public CrosstabBuilder addColumn(String title, String property, String className, boolean showTotal,
        ...
        DJCrosstabColumn col = new CrosstabColumnBuilder().setProperty(property, className).setShowTotals(showTotal)
                .setTitle(title).setHeaderStyle(headerStyle).setTotalHeaderStyle(totalHeaderStyle)
                .setTotalStyle(totalStyle).build();
        addColumn(col);
        return this;
    }

    /**
     * Adds a measure to the crosstab. A crosstab can have many measures. DJ will
     * lay out one measure above the other.
     *
     * A measure is what is shown on each intersection of a column and a row. A
     * calculation is performed to all occurrences in the datasource where the
     * column and row values matches (between elements)
     *
     * The only difference between the prior methods is that this method sets
     * "visible" to false
     *
     * @param property
     * @param className
     ...
     * @return
     */
    public CrosstabBuilder addMeasure(String property, String className, DJCalculation operation, String title,
            Style style) {
    ...
    public CrosstabBuilder addMeasure(String property, String className, DJCalculation operation, String title,
            Style style, boolean isPercentage) {
        DJCrosstabMeasure measure = new DJCrosstabMeasure(property, className, operation, title);
        measure.setStyle(style);
    ...
    public CrosstabBuilder addMeasure(String property, String className, DJCalculation operation, String title,
            Style style, DJValueFormatter valueFormatter, boolean isPercentage) {
        DJCrosstabMeasure measure = new DJCrosstabMeasure(property, className, operation, title);
        measure.setStyle(style);
        measure.setValueFormatter(valueFormatter);
        measure.setIsPercentage(isPercentage);
    ...
    public CrosstabBuilder addMeasure(String property, String className, DJCalculation operation, String title,
            Style style, DJValueFormatter valueFormatter, DJCRosstabMeasurePrecalculatedTotalProvider totalProvider) {
        DJCrosstabMeasure measure = new DJCrosstabMeasure(property, className, operation, title);
        measure.setStyle(style);
        measure.setValueFormatter(valueFormatter);
    ...
    /**
     * Add a row to the crosstab. In a double entry "X\Y" like table table, rows are
     * "X" (columns are Y) The first row added will be the inner most one.
     * 
     * @param row
     * @return
     */
    ...
    public CrosstabBuilder addRow(String title, String property, String className, boolean showTotal) {
        DJCrosstabRow row = new CrosstabRowBuilder().setProperty(property, className).setShowTotals(showTotal)
                .setTitle(title).build();
        addRow(row);
        return this;
    }
    ...
    public CrosstabBuilder addRow(String title, String property, String className, boolean showTotal,
            String totalLegend) {
        DJCrosstabRow row = new CrosstabRowBuilder().setProperty(property, className).setTotalLegend(totalLegend)
                .setShowTotals(showTotal).setTitle(title).build();
        addRow(row);
        return this;
    }
     ...
     * @return
     */
    public CrosstabBuilder addRow(String title, String property, String className, boolean showTotal, Style headerStyle,
            Style totalStyle, Style totalHeaderStyle) {
        DJCrosstabRow row = new CrosstabRowBuilder().setProperty(property, className).setShowTotals(showTotal)
                .setTitle(title).setHeaderStyle(headerStyle).setTotalHeaderStyle(totalHeaderStyle)
                .setTotalStyle(totalStyle).build();
        addRow(row);
    ...
    /**
     * The height if the whole corsstab. This just ensures a minimun height in case
     * the crosstab results shorter that the height specified.
     * 
     * Height is not taken into account if the crosstab must grow because of its
     * data
     * 
     * @param height
     * @return
    ...
    /**
     * To use main report datasource. There should be nothing else in the detail
     * band
     * 
     * @param preSorted
     * @return
    ...
    public CrosstabBuilder useMainReportDatasource(boolean preSorted) {
        DJDataSource datasource = new DJDataSource("ds", DJConstants.DATA_SOURCE_ORIGIN_REPORT_DATASOURCE,
