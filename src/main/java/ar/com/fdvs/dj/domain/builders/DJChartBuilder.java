...
public class DJChartBuilder {
    ...
    public DJChartBuilder addParams(byte type, DJGroup columnsGroup, AbstractColumn column, byte operation,
            DJChartOptions chartOptions) {
        return setType(type).setColumnsGroup(columnsGroup).addColumn(column).setOperation(operation)
    ...
    private DJChartOptions createDefaultOptions() {
        return new DJChartOptions(true, Color.white, 300, 300, true, DJChartOptions.POSITION_HEADER, 0, 0, true,
                (byte) 1, DJChartColors.googleAnalytics());
    }
