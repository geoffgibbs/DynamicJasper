...
/**
 * @author Alejandro Gomez, Juan Alvarez Date: Feb 22, 2007 Time: 4:32:34 PM
 */
public class DJResult extends JasperReportsResult {
    ...
    /**
     * The layout manager to use. Possible values are: classic, list, or a fully
     * qualified java name
     */
    ...
    /**
     * Executes the result given a final location (jsp page, action, etc) and the
     * action invocation (the state in which the action was executed). Subclasses
     * must implement this class to handle custom logic for result handling.
     *
     * @param _finalLocation
     *            the location (jsp page, action, etc) to go to.
     * @param _invocation
    ...
    protected void doExecute(final String _finalLocation, final ActionInvocation _invocation) throws Exception {
        ...
        if ("contype".equals(request.getHeader("User-Agent"))) {
            ...
            if (ds != null) {
                jasperPrint = DynamicJasperHelper.generateJasperPrint(getDynamicReport(_invocation), layoutManagerObj,
                        ds, parameters);
            } else {
                jasperPrint = DynamicJasperHelper.generateJasperPrint(getDynamicReport(_invocation), layoutManagerObj,
                        parameters);
    ...
    /**
     * Returns the export format indicated in the result-type parameter
     * "layoutManager"
     * 
     * @param _invocation
     * @return
    ...
    protected Map getParametersMap(ActionInvocation _invocation) {
        Map map = (Map) _invocation.getStack().findValue(parameters);
        if (map == null) {
            map = new HashMap();
        }
        return map;
    ...
    protected void writeReponse(final HttpServletRequest _request, final HttpServletResponse _response,
            final JasperPrint _jasperPrint, final ActionInvocation _invocation) throws JRException, IOException {
        ...
        final ReportWriter reportWriter = ReportWriterFactory.getInstance().getReportWriter(_jasperPrint,
                documentFormat, parameters);
