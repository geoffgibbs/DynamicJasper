...
public class DynamicJasperHelper {
    ...
    protected static void compileOrLoadSubreports(DynamicReport dr, Map<String, Object> _parameters, String namePrefix)
            throws JRException {
        ...
        for (DJGroup group : dr.getColumnsGroups()) {
            ...
            int subreportNum = 1;
            // Header Subreports
            for (Iterator<Subreport> iterator2 = group.getHeaderSubreports().iterator(); iterator2
                    .hasNext(); subreportNum++) {
                ...
                if (subreport.getDynamicReport() != null) {
                    ...
                    JasperReport jp = generateJasperReport(subreport.getDynamicReport(), subreport.getLayoutManager(),
                            _parameters, name);
            ...
            // Footer Subreports
            subreportNum = 1;
            for (Iterator<Subreport> iterator2 = group.getFooterSubreports().iterator(); iterator2
                    .hasNext(); subreportNum++) {
                Subreport subreport = iterator2.next();
    ...
    public static JasperPrint generateJasperPrint(DynamicReport dr, LayoutManager layoutManager, Collection collection)
            throws JRException {
    ...
    public static JasperPrint generateJasperPrint(DynamicReport dr, LayoutManager layoutManager, Connection con,
            Map<String, Object> _parameters) throws JRException {
        ...
        JRPropertiesUtil.getInstance(DefaultJasperReportsContext.getInstance()).setProperty(JRCompiler.COMPILER_PREFIX,
    ...
    public static JasperPrint generateJasperPrint(DynamicReport dr, LayoutManager layoutManager, ResultSet resultSet)
            throws JRException {
    ...
    /**
     * Compiles the report and applies the layout. <b>generatedParams</b> MUST NOT
     * BE NULL All the key objects from the generatedParams map that are String,
     * will be registered as parameters of the report.
     *
     * @param dr
     * @param layoutManager
     ...
     * @throws JRException
     */
    public static JasperReport generateJasperReport(DynamicReport dr, LayoutManager layoutManager, Map generatedParams)
            throws JRException {
    ...
    public static JasperReport generateJasperReport(DynamicReport dr, LayoutManager layoutManager, Map generatedParams,
            String nameprefix) throws JRException {
        log.info("generating JasperReport with prefix: " + nameprefix);
        JasperReport jr;
        if (generatedParams == null) {
            log.warn(
                    "null parameters map passed to DynamicJasperHelper, you wont be able to retrieve some generated values during the layout process.");
     ...
     * @throws JRException
     */
    public static void generateJRXML(DynamicReport dr, LayoutManager layoutManager, Map _parameters, String xmlEncoding,
    ...
    private static void registerEntities(DynamicJasperDesign jd, DynamicReport dr, LayoutManager layoutManager) {
        ...
        VariableRegistrationManager variableRegistrationManager = new VariableRegistrationManager(jd, dr,
                layoutManager);
        ...
        if (messages == null) {
            try {
                ...
                try {
                    ...
                    throw new DJException("Default messajes file not found: " + DJ_RESOURCE_BUNDLE + "en.properties",
                            e2);
                }
            }
        }
        jd.getParametersWithValues().put(JRParameter.REPORT_RESOURCE_BUNDLE, messages);
        jd.getParametersWithValues().put(JRParameter.REPORT_LOCALE, locale);
    }
        ...
        for (Object key : _parameters.keySet()) {
            if (key instanceof String) {
                try {
                    ...
                    // when using an array as subreport-data-source, I must pass the parameter class
                    // name like this: value.getClass().getComponentType()
    ...
    private static void registerPercentageColumnsVariables(DynamicJasperDesign jd, DynamicReport dr,
        ...
        for (AbstractColumn column : dr.getColumns()) {
            /*
             * Group should not be needed in the percentage column. There should be a
             * variable for each group, using parent group as "rest group"
             */
            if (column instanceof PercentageColumn) {
                ...
                for (DJGroup djGroup : dr.getColumnsGroups()) {
                    ...
                    DJGroupVariableDefRegistrationManager variablesRM = new DJGroupVariableDefRegistrationManager(jd,
                            dr, layoutManager, jrGroup);
     ...
     * ensuring proper subreport with if "fitToParentPrintableArea" flag is set to
     * true
     *
     * @param dr
     * @param _parameters
