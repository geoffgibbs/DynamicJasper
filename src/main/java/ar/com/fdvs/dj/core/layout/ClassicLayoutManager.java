...
public class ClassicLayoutManager extends AbstractLayoutManager {
    ...
    protected static final String EXPRESSION_TRUE_WHEN_NOT_FIRST_PAGE =
            "new java.lang.Boolean(((Number)$V{PAGE_NUMBER}).doubleValue() != 1)";
    ...
    /**
     * Create the image elements for the banners tha goes into the title and header
     * bands depending on the case
     *
     */
    protected void applyBanners() {
        ...
        // if there is no title band, but there are banner images for the first page, we
        // create a title band
    ...
    protected void applyHeaderAutotexts() {
        ...
        // ArrayList<AutoText> autotexts = new
        // ArrayList<AutoText>(getReport().getAutoTexts());
    ...
    protected void applyImageBannersToBand(JRDesignBand band, Collection imageBanners,
            JRDesignExpression printWhenExpression, boolean placeOnTop) {
        int maxHeight = 0;
        if (placeOnTop) {
        ...
        if (band != null) {
            ...
            for (Object imageBanner1 : imageBanners) {
                ...
                if (imageBanner.getAlign() == ImageBanner.Alignment.Left) {
                    ...
                    image.setX(getReport().getOptions().getPage().getWidth() - getReport().getOptions().getLeftMargin()
                            - getReport().getOptions().getRightMargin() - imageBanner.getWidth());
                } else if (imageBanner.getAlign() == ImageBanner.Alignment.Center) {
                    int x = (getReport().getOptions().getPage().getWidth() - getReport().getOptions().getRightMargin()
    ...
    /**
     * Finds the highest sum of height for each possible alignment (left, center,
     * right)
     * 
     * @param aligments
     * @return
    ...
    /**
     * Used to ensure that the general footer label will be at the same Y position
     * as the variables in the band.
     * 
     * @param band
     * @return
     */
    ...
    protected void generateHeaderBand() {
        ...
        /*
         * Note: Te column names, when in header, are printed at the begining of every
         * page. You may dont want this option if you have groups that prints column
         * names.
         */
        ...
        // if
        // (!DynamicJasperHelper.existsGroupWithColumnNames(getReport().getColumnsGroups()))
        // generateHeaderBand(header);
    }
    ...
    /**
     * Returns a list with the columns that are visible. Invisible column are the
     * one whose group is configured with hideColumn = true (in the GroupLayout)
     * 
     * @return
     */
    @Override
    ...
    protected void insertValueInHeader(JRDesignBand headerBand, DJGroup djgroup, int headerOffset) {
        ...
        textField.setStretchType(StretchTypeEnum.NO_STRETCH); // XXX this is a patch for subreports, ensure it works
                                                              // well.
        ...
        for (DJCrosstab djcross : columnsGroup.getHeaderCrosstabs()) {
            ...
            if (caption != null) {
                ...
                if (caption.isJasperExpression()) {
                    ...
                    String invocationText = ExpressionUtils
                            .createCustomExpressionInvocationText(caption.getLabelExpression(), name, false);
                    captExp = ExpressionUtils.createExpression(invocationText,
    ...
    protected void layoutGroupFooterLabels(DJGroup djgroup, JRDesignGroup jgroup, int x, int y, int width, int height) {
        ...
        /*
         * DJGroupVariable lmvar = findLeftMostColumn(footerVariables); AbstractColumn
         * lmColumn = lmvar.getColumnToApplyOperation(); int width =
         * lmColumn.getPosX().intValue() - col.getPosX().intValue();
         * 
        ...
        for (DJGroup columnsGroup : getReport().getColumnsGroups()) {
            ...
            if (columnsGroup.getLayout().isPrintHeaders()) {
                ...
                for (Object o : getVisibleColumns()) {
                    ...
                    designTextField.setPositionType(PositionTypeEnum.FLOAT); // XXX changed to see what happens (must
                                                                             // come from the column position
                                                                             // property)
                    designTextField.setStretchType(StretchTypeEnum.NO_STRETCH); // XXX changed to see what happens (must
                                                                                // come from the column property)
    ...
    protected void layoutGroupSubreports(DJGroup columnsGroup, JRDesignGroup jgroup) {
        ...
        JRDesignBand footerBand = (JRDesignBand) ((JRDesignSection) jgroup.getGroupFooterSection()).getBandsList()
                .get(0);
        JRDesignBand headerBand = (JRDesignBand) ((JRDesignSection) jgroup.getGroupHeaderSection()).getBandsList()
                .get(0);
    ...
    protected void layoutGroupVariables(DJGroup group, JRDesignGroup jgroup, int labelOffset) {
        ...
        if (layout.isShowValueInHeader() && layout.isHideColumn() && !layout.isShowColumnName()) {
            ...
            LayoutUtils.moveBandsElemnts(yOffset - 1, headerBand); // Don't know why, but without the "-1" it wont show
                                                                   // the headers
    ...
    protected void layOutSubReportInBand(DJGroup columnsGroup, JRDesignBand band, String position) {
        ...
        List<Subreport> subreportsList = DJConstants.FOOTER.equals(position) ? columnsGroup.getFooterSubreports()
        ...
        for (Subreport sr : subreportsList) {
            ...
            if (sr.isStartInNewPage()) {
                ...
                if (DJConstants.HEADER.equals(position)) {
                    ...
                    // }
                    // else
                    // targetBand = (JRDesignBand) ((JRDesignGroup)
                    // getDesign().getGroupsList().get(idx-1)).getGroupHeader();
                ...
                /*
                 * There is no meaning in adding a page-break in header sub reports since they
                 * will be placed right after the group header
                 */
            ...
            /*
             * A subreport is placed in a group header or footer. This option configures the
             * group's header/footer band to allow it contents to be split. I'm not sure
             * splitting logic works inside complex object such as sub-reports since it has
             * it's own bands inside
             */
     ...
     * @param yOffset
     */
    protected void placeVariableInBand(List<DJGroupVariable> variables, DJGroup djGroup, JRDesignGroup jgroup,
            String type, JRDesignBand band, int yOffset) {
        ...
        while (it.hasNext()) {
            ...
            if (label != null) {
                ...
                if (label.isJasperExpression()) {
                    ...
                    labelExp = ExpressionUtils.createExpression(variableName + "_labelExpression",
                            label.getLabelExpression(), false);
                } else {
                    // labelExp = ExpressionUtils.createStringExpression("\""+
                    // Utils.escapeTextForExpression(label.getText())+ "\"");
                    labelExp = ExpressionUtils.createStringExpression("\"" + label.getText() + "\"");
            ...
            // First we look for the specific element style, then the default style for the
            // group variables
            ...
            if (var.getStyle() != null) {
                ...
                // the one in the internal cache can get modified by the layout manager (like in
                // the odd row case)
            ...
            if (var.getPrintWhenExpression() != null) {
                JRDesignExpression exp = ExpressionUtils.createExpression(variableName + "_printWhenExpression",
                        var.getPrintWhenExpression(), false);
    ...
    /**
     * page breaks should be near the bottom of the band, this method used while
     * adding subreports which has the "start on new page" option.
     * 
     * @param band
     */
    ...
    /**
     * If a variable has a DJValueFormatter, we must use it in the expression,
     * otherwise, use plain $V{...}
     * 
     * @param expression
     * @param var
     * @param col
     * @param variableName
     */
    protected void setTextAndClassToExpression(JRDesignExpression expression, DJGroupVariable var, AbstractColumn col,
            String variableName) {
