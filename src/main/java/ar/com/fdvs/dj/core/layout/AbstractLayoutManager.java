 ...
 * </br>
 * A Layout Manager is always invoked after the entities registration
 * stage.</br>
...
public abstract class AbstractLayoutManager implements LayoutManager {
    ...
    protected static final String EXPRESSION_TRUE_WHEN_ODD =
            "new java.lang.Boolean(((Number)$V{REPORT_COUNT}).doubleValue() % 2 == 0)";
    ...
    /**
     * Holds the original groups binded to a column. Needed for later reference
     * List<JRDesignGroup>
     */
    ...
    public void addStyleToDesign(Style style) {
        ...
        if (style.getFont() != null && style.getFont().getPdfFontEncoding() == null
                && getReport().getDefaultEncoding() != null) {
     ...
     * used by the first time, it is registered in the jasper-design, if it is the
     * second time, the one created before is used (cached one)
     *
     * @param style
     * @param designElemen
     */
    ...
    protected void applyWaterMark() {
        ...
        ar.com.fdvs.dj.domain.constants.Font font2 = (ar.com.fdvs.dj.domain.constants.Font) djWaterMark.getFont()
                .clone();
        font2.setFontSize(font2.getFontSize() * multiplier);

        BufferedImage watermark = WaterMarkRenderer.rotateText(djWaterMark.getText(), font2.toAwtFont(),
                printableWidth * multiplier, printableHeight * multiplier, djWaterMark.getAngle(),
    ...
    /**
     * Creates the JRDesignChart from the DJChart. To do so it also creates needed
    ...
    protected JRDesignBand createGroupForChartAndGetBand(ar.com.fdvs.dj.domain.chart.DJChart djChart) {
        ...
        // To achieve that, we create a group and insert it right before to the criteria
        // group.
        // I need to find parent group of the criteria group, clone and insert after.
        // The only precaution is that if parent == child (only one group in the report)
        // the we insert before
     ...
     * getExpressionForConditionalStyle(ConditionalStyle condition, AbstractColumn
     * column) { //String text =
     * "(("+CustomExpression.class.getName()+")$P{"+paramName+"})."+CustomExpression
     * .EVAL_METHOD_NAME+"("+textForExpression+")"; String columExpression =
     * column.getTextForExpression();
     * //condition.getCondition().setFieldToEvaluate(exprParams)
     * 
     * // PeS17 patch, 2008-11-29: put all fields to fields map, including
     * "invisible" i.e. only registered ones
     * 
     * String fieldsMap = "(("+DJDefaultScriptlet.class.getName() +
     * ")$P{REPORT_SCRIPTLET}).getCurrentFields()"; String parametersMap =
     * "(("+DJDefaultScriptlet.class.getName() +
     * ")$P{REPORT_SCRIPTLET}).getCurrentParams()"; String variablesMap =
     * "(("+DJDefaultScriptlet.class.getName() +
     * ")$P{REPORT_SCRIPTLET}).getCurrentVariables()";
     * 
     * String evalMethodParams = fieldsMap +", " + variablesMap + ", " +
     * parametersMap + ", " + columExpression;
     * 
     * String text =
     * "(("+ConditionStyleExpression.class.getName()+")$P{"+condition.getName()+
     * "})."+CustomExpression.EVAL_METHOD_NAME+"("+evalMethodParams+")";
     * JRDesignExpression expression = new JRDesignExpression();
     * expression.setValueClass(Boolean.class); expression.setText(text); return
     * expression; }
     */
    ...
    protected JRDesignTextField generateTextFieldFromColumn(AbstractColumn col, int height, DJGroup group) {
        ...
        if (group != null) {
            ...
            // JRDesignGroup previousGroup = (JRDesignGroup)
            // getDesign().getGroupsList().get(index);
            JRDesignGroup previousGroup = getJRGroupFromDJGroup(group);
            textField.setPrintWhenGroupChanges(previousGroup);

            /*
             * Since a group column can share the style with non group columns, if oddRow
             * coloring is enabled, we modified this shared style to have a colored
             * background on odd rows. We don't want that for group columns, that's why we
             * create our own style from the existing one, and remove proper odd-row
             * conditional style if present
             */
             ...
             * (jrstyle.getConditionalStyles() == null ||
             * jrstyle.getConditionalStyles().length == 0)) { // No group column so this is
             * a detail text field JRDesignExpression expression = new JRDesignExpression();
             * expression.setValueClass(Boolean.class);
             * expression.setText(EXPRESSION_TRUE_WHEN_ODD);
             * 
             * Style oddRowBackgroundStyle =
             * getReport().getOptions().getOddRowBackgroundStyle();
             * 
     ...
     * @return The parent group of the given one. If the given one is the first one,
    ...
    protected void layoutCharts() {
        ...
        // Pre-sort charts by group column
        MultiValuedMap<PropertyColumn, ar.com.fdvs.dj.domain.chart.DJChart> mmap2 =
                new ArrayListValuedHashMap<PropertyColumn, ar.com.fdvs.dj.domain.chart.DJChart>();
        ...
        for (PropertyColumn key : mmap2.keySet()) {
            ...
            ArrayList<ar.com.fdvs.dj.domain.chart.DJChart> l = new ArrayList<ar.com.fdvs.dj.domain.chart.DJChart>(
                    charts);
            // Reverse iteration of the charts to meet insertion order
            for (int i = l.size(); i > 0; i--) {
                ...
                if (djChart.getLink() != null) {
                    HyperLinkUtil.applyHyperLinkToElement((DynamicJasperDesign) getDesign(), djChart.getLink(), chart,
                            name + "_hyperlink");
                }
    ...
    protected Map<AbstractColumn, JRDesignVariable> registerChartVariable(ar.com.fdvs.dj.domain.chart.DJChart chart) {
        // FIXME aca hay que iterar por cada columna. Cambiar DJChart para que tome
        // muchas
        ...
        for (Object o : chart.getDataset().getColumns()) {
            ...
            // throw new DJException("Exeption creating chart variable: " +
            // e.getMessage(),e);
            // }
            ...
            // different types of chart from the very same column (with the same operation
            // also) making the variables name to be duplicated
            int chartIndex = getReport().getNewCharts().indexOf(chart);
            var.setName("CHART_[" + chartIndex + "_s" + serieNum + "+]_" + group.getName() + "_" + col.getTitle() + "_"
                    + chart.getOperation());
    ...
    protected void registerRemainingStyles() {
        // TODO: troll all elements in the JRDesing and for elements that has styles
        // with null name
        ...
        for (JRGroup jrgroup : design.getGroupsList()) {
            ...
            if (djGroup != null) {
                for (JRBand headerBand : headerSection.getBandsList()) {
                    setBandFinalHeight((JRDesignBand) headerBand, djGroup.getHeaderHeight(),
                            djGroup.isFitHeaderHeightToContent());

                }
                for (JRBand footerBand : footerSection.getBandsList()) {
                    setBandFinalHeight((JRDesignBand) footerBand, djGroup.getFooterHeight(),
    ...
    /**
     * Sets the columns width by reading some report options like the printableArea
     * and useFullPageWidth. columns with fixedWidth property set in TRUE will not
     * be modified
     */
    ...
    /**
     * set up properly the final JRStyle of the column element (for detail band)
     * upon condition style and odd-background
     *
     * @param jrstyle
     * @param column
     */
    private void setUpConditionStyles(JRDesignStyle jrstyle, AbstractColumn column) {
        ...
        for (ConditionalStyle condition : column.getConditionalStyles()) {
            if (getReport().getOptions().isPrintBackgroundOnOddRows()
                    && Transparency.TRANSPARENT == condition.getStyle().getTransparency()) { // condition style + odd
                                                                                             // row (only if
                                                                                             // conditional style's
                                                                                             // background is
                                                                                             // transparent)
    ...
    protected void transformDetailBand() {
        ...
        // TODO: With this new way, we can use template content as it comes, and add a
        // new band for DJ on top or bellow it.
        JRDesignBand detail;
        ...
        for (AbstractColumn column : getVisibleColumns()) {
            ...
            if (column instanceof BarCodeColumn) {
                ...
                // barcodeColumn.getApplicationIdentifier() + ","+ column.getWidth() +", "+
                ...
                if (column.getLink() != null) {
                    ...
                    HyperLinkUtil.applyHyperLinkToElement((DynamicJasperDesign) getDesign(), column.getLink(), image,
                            name);
                }
             ...
             */
                ...
                if (getReport().getOptions().isShowDetailBand()) {
                    JRDesignTextField textField = generateTextFieldFromColumn(column,
                            getReport().getOptions().getDetailHeight(), null);
