...
public class DJGroupRegistrationManager extends AbstractEntityRegistrationManager {
    ...
    protected void registerEntity(Entity entity) {
        ...
        try {
            ...
            if (djgroup.getName() == null) {
                ...
                String prefix = getDjd().getName() + "_";
                ...
                if (column instanceof GlobalGroupColumn) {
                    ...
                    djgroup.setName(
                            prefix + "group[" + groupIndex + "]_for_column_" + columnIndex + "-" + column.getName());
                }
            }
            ...
            if (djgroup.getFooterLabel() != null && djgroup.getFooterLabel().getLabelExpression() != null) {
                registerCustomExpressionParameter(group.getName() + "_labelExpression",
                        djgroup.getFooterLabel().getLabelExpression());
            }

            // ColumnsGroupFieldVariablesRegistrationManager fieldVariablesRM = new
            // ColumnsGroupFieldVariablesRegistrationManager(getDjd(),getDynamicReport(),getLayoutManager(),
            // group);
            // fieldVariablesRM.registerEntities(djgroup.getFieldVariables());

            DJGroupVariableDefRegistrationManager variablesRM = new DJGroupVariableDefRegistrationManager(getDjd(),
                    getDynamicReport(), getLayoutManager(), group);
            variablesRM.registerEntities(djgroup.getVariables());

            ColumnsGroupVariablesRegistrationManager headerVariablesRM = new ColumnsGroupVariablesRegistrationManager(
                    DJConstants.HEADER, property, getDjd(), getDynamicReport(), getLayoutManager());
            headerVariablesRM.registerEntities(djgroup.getHeaderVariables());

            ColumnsGroupVariablesRegistrationManager footerVariablesRM = new ColumnsGroupVariablesRegistrationManager(
                    DJConstants.FOOTER, property, getDjd(), getDynamicReport(), getLayoutManager());
            footerVariablesRM.registerEntities(djgroup.getFooterVariables());

            DJCrosstabRegistrationManager headerCrosstabsRm = new DJCrosstabRegistrationManager(DJConstants.HEADER,
                    getDjd(), getDynamicReport(), getLayoutManager());
            headerCrosstabsRm.registerEntities(djgroup.getHeaderCrosstabs());

            DJCrosstabRegistrationManager footerCrosstabsRm = new DJCrosstabRegistrationManager(DJConstants.FOOTER,
                    getDjd(), getDynamicReport(), getLayoutManager());
    ...
    /**
     * When a group expression gets its value from a CustomExpression, a variable
     * must be used otherwise it will fail to work as expected.<br>
     * <br>
     * 
     ...
     * @throws JRException
     */
    protected void useVariableForCustomExpression(JRDesignGroup group, JRDesignExpression jrExpression,
            CustomExpression customExpression) throws JRException {
