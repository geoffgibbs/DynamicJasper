...
public class Dj2JrCrosstabBuilder {
    ...
    /**
     * Aplies background coloring upon the matrix described in
     * {@link Dj2JrCrosstabBuilder#createCells}}
     * 
     * @param contents
     * @param crosstabRow
     ...
     * @param cellContent
     * @param topBorder
     *            if true, border settings is applied on all sides. if not, only in
    ...
    /**
     * The max possible width can be calculated doing the sum of of the inner cells
     * and its totals
     * 
     * @param crosstabColumn
     * @return
    ...
    /**
     * The way to create the cells is like this:<br>
     * <br>
     *
     * the result is a matrix of (cols+1)*(rows+1) cells.<br>
     * Each cell has 2 properties that describes which row and column they belong
     * (like coordinates).<br>
     * <br>
     *
     ...
     * <br>
     * where the col(n) is the outer most column, and row(n) is the outer most row
     * in the crosstab<br>
     * <br>
     *
    ...
    protected void createCells() {
        ...
        for (int i = auxCols.length - 1; i >= 0; i--) {
            for (int j = auxRows.length - 1; j >= 0; j--) {
                ...
                for (Iterator<DJCrosstabMeasure> iterator = djcross.getMeasures().iterator(); iterator
                        .hasNext(); measureIdx++, yOffsetCounter++) {
                    DJCrosstabMeasure djmeasure = iterator.next();
                    if (!djmeasure.getVisible()) {
                        yOffsetCounter--;
                        continue; // IMPORTANT! we need to keep the idx to match the index of the measure in the
                                  // measure list
                    }
                    ...
                    if (!isTotalCell) {
                        if (djmeasure.getValueFormatter() == null) {
                            measureExp.setValueClassName(measureValueClassName); // FIXME Shouldn't this be of a class
                                                                                 // "compatible" with measure's
                                                                                 // operation?
                    ...
                    alternateStyle.setName(alternateStyle.getFontName() + "_for_column_"
                            + djmeasure.getProperty().getProperty() + "_i" + i + "_j" + j);
                    ...
                    if (djmeasure.getLink() != null) {
                        ...
                        HyperLinkUtil.applyHyperLinkToElement((DynamicJasperDesign) design, djmeasure.getLink(),
                                element, name);
                    }
    ...
    private void createColumTotalHeader(JRDesignCrosstabColumnGroup ctColGroup, DJCrosstabColumn crosstabColumn,
        ...
        Style totalHeaderstyle = crosstabColumn.getTotalHeaderStyle() == null ? djcross.getColumnTotalheaderStyle()
        ...
        // The height can be the sum of the heights of all the columns starting from the
        // current one, up to the inner most one.
        int auxWidth = 0;
        boolean found = false;
    ...
    private void createRowTotalHeader(JRDesignCrosstabRowGroup ctRowGroup, DJCrosstabRow crosstabRow,
        ...
        ctRowGroup.setTotalPosition(CrosstabTotalPositionEnum.END); // FIXME the total can be at the end of a group or
                                                                    // at the beginin

        Style totalHeaderstyle = crosstabRow.getTotalHeaderStyle() == null ? djcross.getRowTotalheaderStyle()
        ...
        // The width can be the sum of the width of all the rows starting from the
        // current one, up to the inner most one.
        int auxWidth = 0;
        boolean found = false;
        for (DJCrosstabRow row : djcross.getRows()) {
    ...
    private Style getColumnTotalStyle(DJCrosstabColumn crosstabColumnRow) {
        return crosstabColumnRow.getTotalStyle() == null ? djcross.getColumnTotalStyle()
     ...
     * getExpressionForConditionalStyle(ConditionalStyle condition, String
     * columExpression) { String fieldsMap = "(("+DJDefaultScriptlet.class.getName()
     ...
     * 
     * String text = "(("+ConditionStyleExpression.class.getName()+")$P{" +
     * JRParameter.REPORT_PARAMETERS_MAP +
     * "}.get(\""+condition.getName()+"\"))."+CustomExpression.EVAL_METHOD_NAME+"("+evalMethodParams+")";
     * JRDesignExpression expression = new JRDesignExpression();
    ...
    private void registerColumns() {
        for (int i = 0; i < cols.length; i++) {
            ...
            if (crosstabColumn.getOrderByProperty() != null) {
                JRDesignExpression orderByExp = ExpressionUtils.createExpression(
                        "$V{" + crosstabColumn.getOrderByProperty().getProperty() + "}",
            ...
            Style headerstyle = crosstabColumn.getHeaderStyle() == null ? djcross.getColumnHeaderStyle()
    ...
    private void registerMeasures() {
        ...
        for (Iterator<DJCrosstabMeasure> iterator = djcross.getMeasures().iterator(); iterator
            ...
            measure.setName(meausrePrefix + djmeasure.getProperty().getProperty()); // makes the measure.name unique in
                                                                                    // this crosstab
            ...
            if (djmeasure.getIsPercentage()) {
                ...
                try {
                    ...
                    measure.setPercentageCalculatorClassName(JRPercentageCalculatorFactory
                            .getPercentageCalculator(null, valueCass).getClass().getName());
            ...
            if (djmeasure.getValueFormatter() != null) {
                /*
                 * No need to declare parameter in the report design, just in the crosstab,
                 * otherwise duplicated parameter exception may ocurr if many crosstabs are
                 * introduced in the report.
                 */
                // JRDesignParameter dparam = new JRDesignParameter();
                // dparam.setName("crosstab-measure__" + measure.getName() + "_vf"); //value
                // formater suffix
            ...
            if (djmeasure.getPrecalculatedTotalProvider() != null) {
                ...
                JRDesignCrosstabParameter crosstabParameter = new JRDesignCrosstabParameter();
                crosstabParameter.setName("crosstab-measure__" + measure.getName() + "_totalProvider"); // value
                                                                                                        // formater
                                                                                                        // suffix
    ...
    private void registerRows() {
        for (int i = 0; i < rows.length; i++) {
            ...
            if (crosstabRow.getOrderByProperty() != null) {
                JRDesignExpression orderByExp = ExpressionUtils.createExpression(
                        "$V{" + crosstabRow.getOrderByProperty().getProperty() + "}",
            ...
            // The width can be the sum of the with of all the rows starting from the
            // current one, up to the inner most one.
            int auxHeight = getRowHeaderMaxHeight(crosstabRow);
            // int auxHeight = crosstabRow.getHeight(); //FIXME getRowHeaderMaxHeight() must
            // be FIXED because it breaks when 1rs row shows total and 2nd doesn't
            rowTitle.setHeight(auxHeight);

            Style headerstyle = crosstabRow.getHeaderStyle() == null ? djcross.getRowHeaderStyle()
    ...
    /**
     * Sets the options contained in the DJCrosstab to the JRDesignCrosstab. Also
     * fits the correct width
     */
    ...
    /**
     * set proper expression text invoking the
     * DJCRosstabMeasurePrecalculatedTotalProvider for the cell
     * 
     * @param auxRows
     * @param auxCols
     * @param measureExp
    ...
    private void setExpressionForPrecalculatedTotalValue(DJCrosstabColumn[] auxCols, DJCrosstabRow[] auxRows,
        ...
        String expText = "(((" + DJCRosstabMeasurePrecalculatedTotalProvider.class.getName() + ")$P{crosstab-measure__"
                + measureProperty + "_totalProvider}).getValueFor( " + colPropsExp + ", " + colValuesExp + ", "
                + rowPropsExp + ", " + rowValuesExp + " ))";
        ...
        if (djmeasure.getValueFormatter() != null) {
            ...
            // String expText =
            // "((("+DJCRosstabMeasurePrecalculatedTotalProvider.class.getName()+")$P{crosstab-measure__"+djmeasure.getProperty().getProperty()+"_totalProvider}).getValueFor(
            // "
            // + colPropsExp +", "
            // + colValuesExp +", "
            // + rowPropsExp
            ...
            // measureExp.setValueClassName(djmeasure.getValueFormatter().getClassName());
            String valueClassNameForOperation = ExpressionUtils.getValueClassNameForOperation(djmeasure.getOperation(),
                    djmeasure.getProperty());
