...
public class ColumnRegistrationManager extends AbstractEntityRegistrationManager {
    ...
    protected void registerEntity(Entity entity) {
        ...
        if (column.getName() == null) {
            column.setName(getDjd().getName() + "_" + COLUMN_NAME_PREFIX + colCounter++);
        }

        column.setReportName(getDjd().getName());
        ...
        if (column.getConditionalStyles() != null && !column.getConditionalStyles().isEmpty()) {
            ConditionalStylesRegistrationManager conditionalStylesRm = new ConditionalStylesRegistrationManager(
                    getDjd(), getDynamicReport(), column.getName(), getLayoutManager());
            conditionalStylesRm.registerEntities(column.getConditionalStyles());
        }
        if (column.getTextFormatter() != null) {
            JRDesignParameter parameter = new JRDesignParameter();
            parameter.setName(ExpressionUtils.createParameterName("formatter_for_" + column.getName(),
                    column.getTextFormatter()));
        ...
        if (entity instanceof PropertyColumn) {
            try {
                // addField() will throw an exception only if the column has already been
                // registered.
                PropertyColumn propertyColumn = (PropertyColumn) entity;

                ColumnProperty columnProperty = propertyColumn.getColumnProperty();
                if (columnProperty != null
                        && !columnProperty.getProperty().startsWith("__name_to_be_replaced_in_registration_manager_")) {
                ...
                if (entity instanceof ExpressionColumn && ((ExpressionColumn) entity).getExpression() != null) {
                    ...
                    // override property column name to make it unique and determinable. Add report
                    // name as a prefix
                    String property_name = "customExpression_for_" + expressionColumn.getName(); // NOTE Since
                                                                                                 // 3.0.14-beta2
                                                                                                 // Change added to
                                                                                                 // make variable
                                                                                                 // names
                    ...
                    registerCustomExpressionParameter(property_name + "_calc",
    ...
    protected Object transformEntity(Entity entity) {
        ...
        log.debug("Transforming column: " + propertyColumn.getName() + ", property: " + columnProperty.getProperty()
                + " (" + columnProperty.getValueClassName() + ") ");
