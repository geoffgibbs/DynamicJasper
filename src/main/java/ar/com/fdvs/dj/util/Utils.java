...
public class Utils {
    ...
    /**
     * This takes into account objects that breaks the JavaBean convention and have
     * as getter for Boolean objects an "isXXX" method.
     * 
     * @param dest
     * @param orig
     */
    public static void copyProperties(Object dest, Object orig) {
        try {
            if (orig != null && dest != null) {
                ...
                for (PropertyDescriptor origDescriptor : origDescriptors) {
                    ...
                    if (!PropertyUtils.isReadable(orig, name)) { // because of bad convention
                        Method m = orig.getClass().getMethod(
                                "is" + name.substring(0, 1).toUpperCase() + name.substring(1), (Class<?>[]) null);
            ...
            throw new DJException(
                    "Could not copy properties for shared object: " + orig + ", message: " + e.getMessage(), e);
        }
    }

    /**
     * When adding a text to an expression, we have to make sure that the String
     * will not break JasperReports expression syntax.<br>
     * For example: if the title text is: <b>November "2009" sales</b><br>
     * The double quotes (") must be escaped to <b>\"</b>, and in java that would be
     * <b>\\\\\"</b><br>
     *
     * single bars "\" are escaped to "\\"
     *
     * To understand better, the expression can be tested in iReport expression
     * editor.
     * 
     * @param text
     * @return
