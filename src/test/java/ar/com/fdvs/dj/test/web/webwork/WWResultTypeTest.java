...
public class WWResultTypeTest extends TestCase {
    ...
    public void testDynamicReport() throws Exception {
        ServletRunner sr = new ServletRunner(getClass().getResourceAsStream("/webwork/web.xml")); // (1) use the web.xml
                                                                                                  // file to define
                                                                                                  // mappings
        ServletUnitClient client = sr.newClient(); // (2) create a client to invoke the application

        WebResponse r = client.getResponse("http://localhost/generateReport.action"); // (3) invoke the servlet w/o
                                                                                      // authorization
