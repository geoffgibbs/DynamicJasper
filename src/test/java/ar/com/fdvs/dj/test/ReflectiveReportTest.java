...
public class ReflectiveReportTest extends TestCase {
    ...
    public void doReport(final DynamicReport _report, final Collection _data, String name) {
        try {
            final JasperPrint jasperPrint = DynamicJasperHelper.generateJasperPrint(_report, new ClassicLayoutManager(),
                    _data);
            // JasperViewer.viewReport(jasperPrint);
            ReportExporter.exportReport(jasperPrint,
                    System.getProperty("user.dir") + "/target/ReflectiveReportTest " + name + ".pdf");
    ...
    /**
     * Test N�2, the same but we tell the builder the order of the columns, we also
     * add 3 groups
     */
    ...
    /**
     * Test N� 1. With only the collection, the ReflectiveReportBuilder make some
     * guesses
     */
