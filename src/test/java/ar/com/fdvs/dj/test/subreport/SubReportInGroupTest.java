...
public class SubReportInGroupTest extends BaseDjReportTest {
    ...
    private DynamicReport createHeaderSubreport(String title) throws Exception {
        ...
        DynamicReport dr = rb.addColumn("Date", "date", Date.class.getName(), 100)
                ...
                .setUseFullPageWidth(true).setWhenNoDataNoPages().setTitle(title).build();
        return dr;
    }

    private Subreport createSubreport(String title) throws Exception {
        SubReportBuilder srb = new SubReportBuilder();
        srb.setDynamicReport(createHeaderSubreport(title), new ClassicLayoutManager()).setStartInNewPage(true)
                .setDataSource(DJConstants.DATA_SOURCE_ORIGIN_FIELD, DJConstants.DATA_SOURCE_TYPE_COLLECTION,
                        "statistics");
        return srb.build();
    }
