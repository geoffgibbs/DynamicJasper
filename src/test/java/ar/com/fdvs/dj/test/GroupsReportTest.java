...
public class GroupsReportTest extends BaseDjReportTest {
    ...
    public DynamicReport buildReport() throws Exception {
        ...
        drb.setTitleStyle(titleStyle).setTitle("November " + getYear() + " sales report") // defines the title of the
                ...
                .setBottomMargin(margin).setPrintBackgroundOnOddRows(true).setGrandTotalLegend("Grand Total")
                .setGrandTotalLegendStyle(headerVariables).setOddRowBackgroundStyle(oddRowStyle);

        AbstractColumn columnState = ColumnBuilder.getNew().setColumnProperty("state", String.class.getName())
                .setTitle("State").setWidth(new Integer(85)).setStyle(titleStyle).setHeaderStyle(titleStyle).build();
        ...
        AbstractColumn columnaQuantity = ColumnBuilder.getNew().setColumnProperty("quantity", Long.class.getName())
                .setTitle("Quantity").setWidth(new Integer(25)).setStyle(importeStyle).setHeaderStyle(headerStyle)
                .build();

        AbstractColumn columnAmount = ColumnBuilder.getNew().setColumnProperty("amount", Float.class.getName())
                .setTitle("Amount").setWidth(new Integer(100)).setPattern("$ 0.00").setStyle(importeStyle)
        ...
        DJGroup g1 = gb1.setCriteriaColumn((PropertyColumn) columnState)
                .addFooterVariable(columnaQuantity, DJCalculation.SUM, groupVariables) // idem for the columnaQuantity
                                                                                       // column
                // .addFooterVariable(columnAmount,DJCalculation.SUM,groupVariables) // tell the
                // group place a variable footer of the column "columnAmount" with the SUM of
                // allvalues of the columnAmount in this group.
                .addHeaderVariable(columnaQuantity, DJCalculation.SUM, groupVariables) // idem for the columnaQuantity
                                                                                       // column
                .addHeaderVariable(columnAmount, DJCalculation.SUM, groupVariables) // tell the group place a variable
                                                                                    // footer of the column
                                                                                    // "columnAmount" with the SUM of
                                                                                    // allvalues of the columnAmount in
                                                                                    // this group.
                .setGroupLayout(GroupLayout.VALUE_IN_HEADER) // tells the group how to be shown, there are
                                                             // manyposibilities, see the GroupLayout for more.
                .setFooterVariablesHeight(20).setFooterHeight(50, true).setHeaderVariablesHeight(35).build();
