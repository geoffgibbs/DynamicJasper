...
public class ChartReportTest2 extends BaseDjReportTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // JasperDesignViewer.viewReportDesign(DynamicJasperHelper.generateJasperReport(test.dr,
        // new ClassicLayoutManager(),test.params));
    }
    ...
    public DynamicReport buildReport() throws Exception {
        ...
        int margin = 20;
        drb.setProperty(JRPieDataset.PROPERTY_IGNORE_DUPLICATED_KEY, "true").setTitleStyle(titleStyle)
                .setTitle("November " + getYear() + " sales report") // defines the title of the report
                .setSubtitle("The items in this report correspond "
                        + "to the main products: DVDs, Books, Foods and Magazines")
                .setDetailHeight(15).setLeftMargin(margin).setMargins(margin, margin, margin, margin)
                // .setPrintBackgroundOnOddRows(true)
                .setPrintColumnNames(false).setOddRowBackgroundStyle(oddRowStyle);
        ...
        DJGroup g2 = gb2.setCriteriaColumn((PropertyColumn) columnBranch)
                ...
                .addFooterVariable(columnaQuantity, DJCalculation.SUM, g2Variables).build();
        ...
        int chartHeight = 200;
        DJChart chart;
        chart = cb.addType(DJChart.BAR_CHART).addOperation(DJChart.CALCULATION_SUM).addColumnsGroup(g1)
                .addColumn(columnaQuantity).setHeight(chartHeight).setPosition(DJChartOptions.POSITION_HEADER)
                .setShowLabels(true).build();

        drb.addChart(chart); // add chart

        // 2nd chart
        cb = new DJChartBuilder();
        chart = cb.addType(DJChart.BAR_CHART).addOperation(DJChart.CALCULATION_SUM).addColumnsGroup(g2)
                .setHeight(chartHeight).addColumn(columnaQuantity).addColumn(columnAmount)
                .setPosition(DJChartOptions.POSITION_HEADER).setShowLabels(true).build();

        drb.addChart(chart); // add chart

        // 3rd Chart
        cb = new DJChartBuilder();
        chart = cb.addType(DJChart.PIE_CHART).addOperation(DJChart.CALCULATION_SUM).addColumnsGroup(g2)
                .addColumn(columnaQuantity).setHeight(chartHeight).setPosition(DJChartOptions.POSITION_HEADER).build();
