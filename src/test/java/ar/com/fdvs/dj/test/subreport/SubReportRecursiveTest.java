...
public class SubReportRecursiveTest extends BaseDjReportTest {
    ...
    private DynamicReport createLevel2Subreport() throws Exception {
        ...
        DynamicReport dr = rb.addColumn("Date", "date", Date.class.getName(), 100)
                ...
                .addField("dummy3", Collection.class.getName()).setMargins(5, 5, 20, 20).setUseFullPageWidth(true)
    ...
    private DynamicReport createLevel3Subreport() throws Exception {
        FastReportBuilder rb = new FastReportBuilder();
        DynamicReport dr = rb.addColumn("Name", "name", String.class.getName(), 100)
                .addColumn("Number", "number", Long.class.getName(), 50).setMargins(5, 5, 20, 20)
                .setUseFullPageWidth(false).setTitle("Level 3 Subreport").build();
        return dr;
    }
