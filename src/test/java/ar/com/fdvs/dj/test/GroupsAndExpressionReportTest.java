...
public class GroupsAndExpressionReportTest extends BaseDjReportTest {
    ...
    public DynamicReport buildReport() throws Exception {
        ...
        AbstractColumn columnState = ColumnBuilder.getNew().setColumnProperty("state", String.class.getName())
                .setTitle("State").setWidth(new Integer(100)).setStyle(detailStyle).setHeaderStyle(titleStyle).build();
        ...
        AbstractColumn exprCol = ColumnBuilder.getNew().setCustomExpression(customExpression).build();

        GroupBuilder gb1 = new GroupBuilder();

        // define the criteria column to group by (columnState)
        DJGroup g1 = gb1.setCriteriaColumn((PropertyColumn) exprCol).setGroupLayout(GroupLayout.EMPTY)
                .setStartInNewPage(true).build();
