...
public class Struts2ResultTypeTest extends TestCase {
    ...
    public void testDynamicReport() throws Exception {
        ServletRunner sr = new ServletRunner(getClass().getResourceAsStream("/struts2/web.xml")); // (1) use the web.xml
                                                                                                  // file to define
                                                                                                  // mappings
        ServletUnitClient client = sr.newClient(); // (2) create a client to invoke the application

        WebResponse r = client.getResponse("http://localhost/example/generateReport.action"); // (3) invoke the servlet
                                                                                              // w/o authorization
    ...
    public void testDynamicReportWithTemplate() throws Exception {
        ...
        WebResponse r = client.getResponse("http://localhost/generateReportTemplate.action"); // (3) invoke the servlet
                                                                                              // w/o authorization
        String fname = r.getHeaderField("CONTENT-DISPOSITION");
        fname = fname.substring(fname.indexOf("=") + 1);
