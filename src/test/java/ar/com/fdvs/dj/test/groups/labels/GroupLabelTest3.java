...
public class GroupLabelTest3 extends BaseDjReportTest {
    ...
    public DynamicReport buildReport() throws Exception {
        ...
        drb.setTitleStyle(titleStyle).setTitle("November " + getYear() + " sales report") // defines the title of the
                ...
                .setBottomMargin(margin).setPrintBackgroundOnOddRows(false).setGrandTotalLegend("Grand Total")
                .setGrandTotalLegendStyle(headerVariables).setDefaultStyles(titleStyle, null, headerStyle, detailStyle)
                .setPrintColumnNames(false)
                .addImageBanner(
                        System.getProperty("user.dir") + "/target/test-classes/images/logo_fdv_solutions_60.png", 100,
                        30, ImageBanner.Alignment.Right)
        ...
        ColumnBuilder.getNew().setColumnProperty("item", String.class.getName()).setTitle("Item")
                .setWidth(new Integer(85)).setStyle(detailStyle).setHeaderStyle(headerStyle).build();

        ColumnBuilder.getNew().setColumnProperty("id", Long.class.getName()).setTitle("ID").setWidth(new Integer(40))
        ...
        AbstractColumn columnAmount = ColumnBuilder.getNew().setColumnProperty("amount", Float.class.getName())
                .setTitle("Amount").setWidth(new Integer(90)).setPattern("$ 0.00").setStyle(importeStyle)
                .setHeaderStyle(headerStyle).build();

        GroupBuilder gb1 = new GroupBuilder();

        Style glabelStyle2 = new StyleBuilder(false).setFont(Font.ARIAL_SMALL).setHorizontalAlign(HorizontalAlign.RIGHT)
                .setVerticalAlign(VerticalAlign.MIDDLE).setPadding(0).setStretchWithOverflow(false).build();

        DJGroupLabel glabel3 = new DJGroupLabel("Subtotal", glabelStyle2, LabelPosition.TOP);

        // define the criteria column to group by (columnState)
        DJGroup g1 = gb1.setCriteriaColumn((PropertyColumn) columnState)
                // .addHeaderVariable(columnAmount,DJCalculation.SUM,headerVariables) // tell
                // the group place a variable footer of the column "columnAmount" with the SUM
                // of allvalues of the columnAmount in this group.
                // .addHeaderVariable(columnaQuantity,DJCalculation.SUM,headerVariables) // idem
                // for the columnaQuantity column
                .addFooterVariable(columnAmount, DJCalculation.SUM, headerVariables) // tell the group place a variable
                                                                                     // footer of the column
                                                                                     // "columnAmount" with the SUM
                                                                                     // of allvalues of the
                                                                                     // columnAmount in this group.
                .addFooterVariable(columnaQuantity, DJCalculation.SUM, headerVariables) // idem for the columnaQuantity
                                                                                        // column
                // .setGroupLayout(GroupLayout.VALUE_IN_HEADER_WITH_HEADERS) // tells the group
                // how to be shown, there are manyposibilities, see the GroupLayout for more.
                .setGroupLayout(GroupLayout.VALUE_IN_HEADER_WITH_HEADERS) // tells the group how to be shown, there are
                                                                          // manyposibilities, see the GroupLayout for
                                                                          // more.
        ...
        gb2.setCriteriaColumn((PropertyColumn) columnBranch) // and we add the same operations for the columnAmount and
